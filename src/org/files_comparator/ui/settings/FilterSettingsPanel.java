...
import org.files_comparator.settings.FilterSettings;
import org.files_comparator.settings.util.Filter;
import org.files_comparator.settings.util.FilterRule;
import org.files_comparator.ui.swing.table.FilesComparatorTableModel;
import org.files_comparator.ui.swing.table.util.FilesComparatorComboBoxEditor;
import org.files_comparator.ui.swing.table.util.FilesComparatorComboBoxRenderer;
 ...
 * 
 */
public class FilterSettingsPanel extends FilterSettingsForm
        implements
            ConfigurationListenerIF {

    private static final long serialVersionUID = -7881185351946639262L;

    FilesComparatorTableModel filterTableModel;
    FilesComparatorTableModel filterRuleTableModel;
    ...
    private void init() {
        ...
        filterRuleTable.setDefaultEditor(Filter.class,
                new FilesComparatorComboBoxEditor(getFilters()));
        filterRuleTable.setDefaultRenderer(Filter.class,
                new FilesComparatorComboBoxRenderer(getFilters()));
        filterRuleTable.setDefaultEditor(FilterRule.Rule.class,
                new FilesComparatorComboBoxEditor(FilterRule.Rule.values()));
        filterRuleTable.setDefaultRenderer(FilterRule.Rule.class,
                new FilesComparatorComboBoxRenderer(FilterRule.Rule.values()));
    ...
    private FilesComparatorTableModel getFilterTableModel() {
        return new FilterTableModel();
    }

    private FilesComparatorTableModel getFilterRuleTableModel(int filterIndex) {
    ...
    private class FilterRuleTableModel extends FilesComparatorTableModel {
        ...
        private void init() {
            activeColumn = addColumn("active", null, "Active", Boolean.class,
                    5, true);
            descriptionColumn = addColumn("description", null, "Description",
                    String.class, 15, true);
            ruleColumn = addColumn("rule", null, "Rule", FilterRule.Rule.class,
                    10, true);
            patternColumn = addColumn("pattern", null, "Pattern", String.class,
                    -1, true);
        }
        ...
        public Class getColumnClass(int rowIndex, Column column) {
            ...
            if (column == patternColumn) {
                rule = getRule(rowIndex);
                if (rule != null
                        && rule.getRule() == FilterRule.Rule.importFilter) {
                    return Filter.class;
                }
            }
    ...
    private FilterSettings getFilterSettings() {
        return FilesComparatorSettings.getInstance().getFilter();
    }
}