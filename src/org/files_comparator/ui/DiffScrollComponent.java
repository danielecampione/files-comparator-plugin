 ...
 *
 */
public class DiffScrollComponent extends JComponent
        implements
            ChangeListener,
            ConfigurationListenerIF {

    private static final long serialVersionUID = 6393577020361983817L;
        ...
        try {
            ...
            for (FilesComparatorDelta delta : revision.getDeltas()) {
                ...
                if (selected) {
                    ...
                    if (height > 0) {
                        g2.drawLine(x + selectionWidth, y, x + selectionWidth,
                                y + height);
                        g2.drawLine(x, y + height, x + selectionWidth, y
                                + height);
                    }
                ...
                if (!leftsideReadonly && !bdFrom.isReadonly()) {
                    ...
                    g2.draw(shape);
                    resetAntiAlias(g2);
                    commands.add(new DiffChangeCommand(shape, delta,
                            toPanelIndex, fromPanelIndex));

                    // Draw delete right command
                    if (original.getSize() > 0) {
                        g2.setColor(Color.red);
                        g2.drawLine(x0 + 3 - width, y0 + 3, x0 + 7 - width,
                                y0 + 7);
                        g2.drawLine(x0 + 7 - width, y0 + 3, x0 + 3 - width,
                                y0 + 7);
                        rect = new Rectangle(x0 + 2 - width, y0 + 2, 6, 6);
                        commands.add(new DiffDeleteCommand(rect, delta,
                                fromPanelIndex, toPanelIndex));
                    }
                }
    ...
    class DiffChangeCommand extends Command {
        DiffChangeCommand(Shape shape, FilesComparatorDelta delta,
    ...
    class DiffDeleteCommand extends Command {
        DiffDeleteCommand(Shape shape, FilesComparatorDelta delta,
                int fromIndex, int toIndex) {
            super(shape, delta, fromIndex, toIndex);
        }
    ...
    abstract class Command {
        ...
        Command(Shape shape, FilesComparatorDelta delta, int fromIndex,
                int toIndex) {
