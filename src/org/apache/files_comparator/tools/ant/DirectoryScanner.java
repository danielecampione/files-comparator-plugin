...
import org.apache.files_comparator.tools.ant.types.ResourceFactory;
import org.apache.files_comparator.tools.ant.types.selectors.FileSelector;
import org.apache.files_comparator.tools.ant.types.selectors.SelectorScanner;
import org.apache.files_comparator.tools.ant.types.selectors.SelectorUtils;
import org.apache.files_comparator.tools.ant.util.FileUtils;
            ...
            synchronized (this) {
                for (String excluded : dirsExcluded) {
                    if (!couldHoldIncluded(excluded)) {
                        scandir(new File(basedir, excluded), excluded
            ...
            for (int i = 0; i < newfiles.length; i++) {
                try {
                    if (FILE_UTILS.isSymbolicLink(dir, newfiles[i])) {
                        ...
                        (file.isDirectory() ? dirsExcluded : filesExcluded)
                                .add(name);
                    } else {
        ...
        for (String exclude : excludes) {
            if (exclude.endsWith("**")
                    && SelectorUtils.matchPath(
                            exclude.substring(0, exclude.length() - 2), name,
                            isCaseSensitive())) {
                return true;
            }
        }
